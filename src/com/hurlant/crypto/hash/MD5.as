 	...
 	public class MD5 implements IHash
	{
		...
		private function core_md5(x:Array, len:uint):Array {
		  ...
		  for(var i:uint = 0; i < x.length; i += 16)
		  {
		    ...
		    a = RPC_XML_Demo(a, b, c, d, x[i+ 0], 7 , 0xD76AA478);
		    d = RPC_XML_Demo(d, a, b, c, x[i+ 1], 12, 0xE8C7B756);
		    c = RPC_XML_Demo(c, d, a, b, x[i+ 2], 17, 0x242070DB);
		    b = RPC_XML_Demo(b, c, d, a, x[i+ 3], 22, 0xC1BDCEEE);
		    a = RPC_XML_Demo(a, b, c, d, x[i+ 4], 7 , 0xF57C0FAF);
		    d = RPC_XML_Demo(d, a, b, c, x[i+ 5], 12, 0x4787C62A);
		    c = RPC_XML_Demo(c, d, a, b, x[i+ 6], 17, 0xA8304613);
		    b = RPC_XML_Demo(b, c, d, a, x[i+ 7], 22, 0xFD469501);
		    a = RPC_XML_Demo(a, b, c, d, x[i+ 8], 7 , 0x698098D8);
		    d = RPC_XML_Demo(d, a, b, c, x[i+ 9], 12, 0x8B44F7AF);
		    c = RPC_XML_Demo(c, d, a, b, x[i+10], 17, 0xFFFF5BB1);
		    b = RPC_XML_Demo(b, c, d, a, x[i+11], 22, 0x895CD7BE);
		    a = RPC_XML_Demo(a, b, c, d, x[i+12], 7 , 0x6B901122);
		    d = RPC_XML_Demo(d, a, b, c, x[i+13], 12, 0xFD987193);
		    c = RPC_XML_Demo(c, d, a, b, x[i+14], 17, 0xA679438E);
		    b = RPC_XML_Demo(b, c, d, a, x[i+15], 22, 0x49B40821);
		...
		private function RPC_XML_Demo(a:uint, b:uint, c:uint, d:uint, x:uint, s:uint, t:uint):uint {
